	...
	$(document).ready(function(){
		...
		jQuery('#ampaginationsm').on('am.pagination.change',function(e){
			...
			if(search == "search"){
		        ...
		        $(location).attr('href', "http://localhost:9000/notice/admin_event_list_search.do?rpage="+e.page);
			}else{
				jQuery('.showlabelsm').text('The selected page no: '+e.page);
		       $(location).attr('href', "http://localhost:9000/notice/admin_event_list.do?rpage="+e.page);
			}
	    });
					...
					<!-- 일반게시판_List -->
						<table class="contents_table">
							<td colspan="4">
								...
								<tr>
									<td>${vo.rno }</td>
									<td><a href="admin_event_content.do?nid=${vo.nid }">${vo.ntitle }</a></td>
									<td>${vo.nhits }</td>
									<td>${vo.ndate }</td>
								</tr>
								</c:forEach>
